...
if __name__ == "__main__" :
    ...
    indir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_/hdf5dataframes/DMsorted'
    mapdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_/pklModels/mapping'
    plotdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_/plots/rates'
    os.system('mkdir -p '+plotdir)

    # define the input and output dictionaries for the handling of different datasets
    inFileNu_dict = {
        'threshold'    : indir+'/Nu_PU200_th_PUWP{0}_ISOWP{1}_DMsorted.hdf5'.format(args.PUWP,args.ISOWP),
        'supertrigger' : indir+'/',
        'bestchoice'   : indir+'/',
        'bestcoarse'   : indir+'/',
        'mixed'        : indir+'/'
    ...
    inFileTau_mapping_dict = {
        'threshold'    : mapdir+'/Tau_PU200_th_isolation_PUWP{0}_ISOWP{1}_mapping.pkl'.format(args.PUWP,args.ISOWP),
        'supertrigger' : mapdir+'/',
        'bestchoice'   : mapdir+'/',
        'bestcoarse'   : mapdir+'/',
        'mixed'        : mapdir+'/'
    ...
    for name in feNames_dict:
        ...
        df4Rate.query('cl3d_pt_c3>10 and cl3d_pubdt_passWP{0}==True and cl3d_isobdt_passWP{1}==True'.format(args.PUWP,args.ISOWP), inplace=True)
        events_passing[name] = df4Rate.reset_index().duplicated(['event']).sum()
...
'''
        ...
        # DM0 DATAFRAMES
        tmp_DM0 = df4Rate.query('cl3d_predDM==0').copy(deep=True)
        ...
        # DM1 DATAFRAMES
        tmp_DM1 = df4Rate.query('cl3d_predDM==1'.format(args.PUWP)).copy(deep=True)
        ...
        # DM2 DATAFRAMES
        tmp_DM2 = df4Rate.query('cl3d_predDM==2').copy(deep=True)
        ...
        #tmp_DM3 = df4Rate.query('cl3d_predDM==3').copy(deep=True)
...
plt.ylabel('Rate [kHz]')
plt.title('Rate - PUWP={0} ISOWP={1}'.format(args.PUWP,args.ISOWP), fontsize=15)
plt.subplots_adjust(bottom=0.12)
plt.savefig(plotdir+'/rate_offline_Nu_isolated_PUWP{0}_ISOWP{1}.pdf'.format(args.PUWP,args.ISOWP))
plt.close()
...
plt.title('Rate - PUWP={0} ISOWP={1}'.format(args.PUWP,args.ISOWP), fontsize=15)
plt.subplots_adjust(bottom=0.12)
plt.savefig(plotdir+'/rate_online_isolated_PUWP{0}_ISOWP{1}.pdf'.format(args.PUWP,args.ISOWP))
plt.close()
