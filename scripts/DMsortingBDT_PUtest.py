...
if __name__ == "__main__" :
    ...
    outFileTau_dict = {
        'threshold'    : outdir+'/RelValTenTau_PU200_th_DMsorted.hdf5',
        'supertrigger' : outdir+'/',
        'bestchoice'   : outdir+'/',
        'bestcoarse'   : outdir+'/',
        'mixed'        : outdir+'/'
    ...
    outFileNu_dict = {
        'threshold'    : outdir+'/RelValNu_PU200_th_DMsorted.hdf5',
        'supertrigger' : outdir+'/',
        'bestchoice'   : outdir+'/',
        'bestcoarse'   : outdir+'/',
        'mixed'        : outdir+'/'
    ...
    outFileHH_dict = {
        'threshold'    : outdir+'/GluGluHHTo2b2Tau_PU200_th_DMsorted.hdf5',
        'supertrigger' : outdir+'/',
        'bestchoice'   : outdir+'/',
        'bestcoarse'   : outdir+'/',
        'mixed'        : outdir+'/'
    ...
    outFileQCD_dict = {
        'threshold'    : outdir+'/QCD_PU200_th_DMsorted.hdf5',
        'supertrigger' : outdir+'/',
        'bestchoice'   : outdir+'/',
        'bestcoarse'   : outdir+'/',
        'mixed'        : outdir+'/'
    ...
    outFile_model_dict = {
        'threshold'    : model_dict_outdir+'/model_DMsorting_th_PU200.pkl',
    ...
    for name in feNames_dict:
        ...
        store_nu.close()   
        dfNu_dict[name]['gentau_decayMode'] = -1 # tag as PU
        ...
        dfTauTraining_dict[name] = dfTauTraining_dict[name][genPt_sel & eta_sel1 & eta_sel2 & cl3dBest_sel & cl3dPt_sel & DMsel]
        ...
        # apply slections for the training dataset
        dfQCDTraining_dict[name] = dfQCDTraining_dict[name][genPt_sel & eta_sel1 & eta_sel2 & cl3dBest_sel & cl3dPt_sel]
        ...
        # MERGE
        dfMergedTraining_dict[name] = pd.concat([dfTauTraining_dict[name],dfNuTraining_dict[name],dfQCDTraining_dict[name]],sort=False)
        dfMergedValidation_dict[name] = pd.concat([dfHHValidation_dict[name],dfNuValidation_dict[name],dfQCDValidation_dict[name]],sort=False)
        ...
        # replace DMs with category numbers (REMEMBER: category number 4 is QCD)
        dfMergedTraining_dict[name]['gentau_decayMode'].replace([0,5,10], 0, inplace=True) # prong decays
        dfMergedTraining_dict[name]['gentau_decayMode'].replace([1], 1, inplace=True) # 1-prong+pi decays
        dfMergedTraining_dict[name]['gentau_decayMode'].replace([11], 2, inplace=True) # 3-prong+pi decays        
        dfMergedTraining_dict[name]['gentau_decayMode'].replace([4], 3, inplace=True) # qcd
        dfMergedTraining_dict[name]['gentau_decayMode'].replace([-1], 4, inplace=True) # pileup

        dfMergedValidation_dict[name]['gentau_decayMode'].replace([0,5,10], 0, inplace=True) # prong decays
        dfMergedValidation_dict[name]['gentau_decayMode'].replace([1], 1, inplace=True) # 1-prong+pi decays
        dfMergedValidation_dict[name]['gentau_decayMode'].replace([11], 2, inplace=True) # 3-prong+pi decays
        dfMergedValidation_dict[name]['gentau_decayMode'].replace([4], 3, inplace=True) # qcd
        dfMergedValidation_dict[name]['gentau_decayMode'].replace([-1], 4, inplace=True) # pileup
        ...
        dfTau_dict[name]['gentau_decayMode'].replace([0,5,10], 0, inplace=True)
        dfTau_dict[name]['gentau_decayMode'].replace([1], 1, inplace=True)
        dfTau_dict[name]['gentau_decayMode'].replace([11], 2, inplace=True)

        dfHH_dict[name]['gentau_decayMode'].replace([0,5,10], 0, inplace=True)
        dfHH_dict[name]['gentau_decayMode'].replace([1], 1, inplace=True)
        dfHH_dict[name]['gentau_decayMode'].replace([11], 2, inplace=True)

        dfQCD_dict[name]['gentau_decayMode'].replace([4], 3, inplace=True)
        dfNu_dict[name]['gentau_decayMode'].replace([-1], 4, inplace=True)
        ...
        plt.savefig(plotdir+'/DM_roc_'+name+'_training.pdf')
        plt.close()
        ...
        plt.savefig(plotdir+'/DM_roc_'+name+'_validation.pdf')
        plt.close()
        ...
        if args.doPlots:
            for name in feNames_dict:
                ...
                for var in features_dict:
                    ...
                    plt.hist(dfNu_dict[name][var], density=True, bins=np.arange(features_dict[var][1][0],features_dict[var][1][1],(features_dict[var][1][1]-features_dict[var][1][0])/features_dict[var][1][2]), label='PU', color='olive',  histtype='step', lw=2, alpha=0.8)
                    ...
                    PU_line = mlines.Line2D([], [], color='olive', markersize=15, label='PU', lw=2)                    
                    plt.legend(loc = 'upper right',handles=[DM0_line,DM1_line,DM5_line,DM11_line,QCD_line,PU_line])
                    plt.grid(linestyle=':')
                ...
                plt.xticks([0,1,2,3,4], ('prong', r'1-prong$+\pi_0$',r'3-prong$+\pi_0$', 'QCD', 'PU'))
                plt.yticks([0,1,2,3,4], ('prong', r'1-prong$+\pi_0$',r'3-prong$+\pi_0$', 'QCD', 'PU'))
                plt.xlim(-0.5, 4.5)
                plt.ylim(-0.5, 4.5),
                ...
                plt.savefig(plotdir+'/DMconfMatrix_'+name+'.pdf')
                plt.close()
                ...
                plt.savefig(plotdir+'/DM0_prob_'+feNames_dict[name]+'.pdf')
                plt.close()
                ...
                plt.ylabel(r'Entries')
                plt.savefig(plotdir+'/DM1_prob_'+name+'.pdf')
                plt.close()
                ...
                plt.ylabel(r'Entries')
                plt.savefig(plotdir+'/DM2_prob_'+name+'.pdf')
                plt.close()
                ...
                plt.ylabel(r'Entries')
                plt.savefig(plotdir+'/DMqcd_prob_'+name+'.pdf')
                plt.close()
                ...
                plt.ylabel(r'Entries')
                plt.savefig(plotdir+'/DMpu_prob_'+name+'.pdf')
                plt.close()
                ...
                # plt.savefig(plotdir+'/DM_roc_'+name+'.pdf')
                # plt.close()
