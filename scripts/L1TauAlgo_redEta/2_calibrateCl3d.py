...
if __name__ == "__main__" :
    ...
    indir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/hdf5dataframes/matched'
    outdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/hdf5dataframes/calibrated_C1fullC2C3'
    plotdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/plots/calibration_C1fullC2C3'
    model_outdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/pklModels/calibration_C1fullC2C3'
    ...
    for name in feNames_dict:
        ...
        dfTraining_dict[name].query('(cl3d_pt>{0} and cl3d_abseta>1.6 and cl3d_abseta<2.1) or ((gentau_vis_pt>20 and ((gentau_vis_eta>1.6 and gentau_vis_eta<2.1) or (gentau_vis_eta<-1.6 and gentau_vis_eta>-2.1))) or (genjet_pt>20 and ((genjet_eta>1.6 and genjet_eta<2.1) or (genjet_eta<-1.6 and genjet_eta>-2.1))))'.format(args.ptcut), inplace=True)
        dfValidation_dict[name].query('(cl3d_pt>{0} and cl3d_abseta>1.6 and cl3d_abseta<2.1) or ((gentau_vis_pt>20 and ((gentau_vis_eta>1.6 and gentau_vis_eta<2.1) or (gentau_vis_eta<-1.6 and gentau_vis_eta>-2.1))) or (genjet_pt>20 and ((genjet_eta>1.6 and genjet_eta<2.1) or (genjet_eta<-1.6 and genjet_eta>-2.1))))'.format(args.ptcut), inplace=True)

        # for the actual training and validation we enforce the AND between jet and cluster requirements
        dfQCDTr = dfTraining_dict[name].query('gentau_decayMode==-2 and cl3d_isbestmatch==True and (genjet_pt>20 and ((genjet_eta>1.6 and genjet_eta<2.1) or (genjet_eta<-1.6 and genjet_eta>-2.1)))').copy(deep=True)
        dfQCDVal = dfValidation_dict[name].query('gentau_decayMode==-2 and cl3d_isbestmatch==True and (genjet_pt>20 and ((genjet_eta>1.6 and genjet_eta<2.1) or (genjet_eta<-1.6 and genjet_eta>-2.1)))').copy(deep=True)

        # for the actual training and validation we enforce the AND between tau and cluster requirements
        dfTr = dfTraining_dict[name].query('gentau_decayMode>=0 and cl3d_isbestmatch==True and (gentau_vis_pt>20 and ((gentau_vis_eta>1.6 and gentau_vis_eta<2.1) or (gentau_vis_eta<-1.6 and gentau_vis_eta>-2.1)))').copy(deep=True)
        dfVal = dfValidation_dict[name].query('gentau_decayMode>=0 and cl3d_isbestmatch==True and (gentau_vis_pt>20 and ((gentau_vis_eta>1.6 and gentau_vis_eta<2.1) or (gentau_vis_eta<-1.6 and gentau_vis_eta>-2.1)))').copy(deep=True)
