...
if __name__ == "__main__" :
    ...
    parser.add_argument('--doOnlyRate', dest='doOnlyRate', help='L1 algorithm already applied, jump to rate evluation directly', action='store_true', default=False)
    ...
    if not args.doOnlyRate:
        # create needed folders
        indir   = '/data_CMS_upgrade/motta/HGCAL_SKIMS/SKIM_12May2021'
        outdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/hdf5dataframes/rateEvluation'
        ...
        outFileMinbias = {
            'threshold'    : outdir+'/Minbias_PU200_th_rateEvaluated.hdf5',
            'mixed'        : outdir+'/'
        }
        ...
        calib_model_indir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/pklModels/calibration_C1fullC2C3'
        ...
        PUrej_model_indir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/pklModels/PUrejection_C1fullC2C3_fullPUnoPt'
        ...
        isoQCDrej_model_indir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/pklModels/isolation_C1fullC2C3_fullPUnoPt_fullISO'

        model_isoQCDrej = {
            'threshold'    : isoQCDrej_model_indir+'/model_isolation_PUWP{0}_th_PU200.pkl',
            'mixed'        : isoQCDrej_model_indir+'/'
        }

        WP15_isoQCDrej = {
            'threshold'    : isoQCDrej_model_indir+'/WP15_isolation_PUWP{0}_th_PU200.pkl',
            'mixed'        : isoQCDrej_model_indir+'/'
        }

        WP10_isoQCDrej = {
            'threshold'    : isoQCDrej_model_indir+'/WP10_isolation_PUWP{0}_th_PU200.pkl',
            'mixed'        : isoQCDrej_model_indir+'/'
        }

        WP05_isoQCDrej = {
            'threshold'    : isoQCDrej_model_indir+'/WP05_isolation_PUWP{0}_th_PU200.pkl',
            'mixed'        : isoQCDrej_model_indir+'/'
        }

        WP01_isoQCDrej = {
            'threshold'    : isoQCDrej_model_indir+'/WP01_isolation_PUWP{0}_th_PU200.pkl',
            'mixed'        : isoQCDrej_model_indir+'/'
        }

        # DICTIONARIES FOR THE DM SORTING
        DMsort_model_indir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/pklModels/DMsorting_C1fullC2C3_fullPUnoPt_fullISO'
        ...
        df4Rate.query('cl3d_pt>4. and cl3d_abseta>1.6 and cl3d_abseta<2.1', inplace=True) #JONA eta cut
        ...
        features_calib = ['cl3d_showerlength', 'cl3d_coreshowerlength', 'cl3d_firstlayer', 'cl3d_seetot', 'cl3d_seemax', 'cl3d_spptot', 'cl3d_sppmax', 'cl3d_szz', 'cl3d_srrtot', 'cl3d_srrmax', 'cl3d_srrmean', 'cl3d_hoe', 'cl3d_meanz']
        ...
        #######################################################################################################################################
        # PILE UP REJECTION
        #features_PUrej = ['cl3d_c1', 'cl3d_c2', 'cl3d_c3', 'cl3d_pt_c3', 'cl3d_abseta', 'cl3d_showerlength', 'cl3d_coreshowerlength', 'cl3d_firstlayer', 'cl3d_seetot', 'cl3d_seemax', 'cl3d_spptot', 'cl3d_sppmax', 'cl3d_szz', 'cl3d_srrtot', 'cl3d_srrmax', 'cl3d_srrmean', 'cl3d_hoe', 'cl3d_meanz']
        features_PUrej = ['cl3d_abseta', 'cl3d_showerlength', 'cl3d_coreshowerlength', 'cl3d_firstlayer', 'cl3d_seetot', 'cl3d_spptot', 'cl3d_szz', 'cl3d_srrtot', 'cl3d_hoe', 'cl3d_meanz']
        ...
        dfL1Candidates = df4Rate.query('cl3d_pubdt_passWP99==True').copy(deep=True) # selecting WP 99 we select also 95 and 90 --> the selection on the specific WP is applied when calculating turnONs
        ...
        for PUWP in ['99', '95', '90']:
            ...
            ISObdtWP15 = load_obj(WP15_isoQCDrej[args.FE].format(PUWP))
            full = xgb.DMatrix(data=df4Rate[features_isoQCDrej], feature_names=features_isoQCDrej)
            df4Rate['cl3d_isobdt_score'] = model_iso.predict(full)
            df4Rate['cl3d_isobdt_passWP01_PUWP{0}'.format(PUWP)] = df4Rate['cl3d_isobdt_score'] > ISObdtWP01
            df4Rate['cl3d_isobdt_passWP05_PUWP{0}'.format(PUWP)] = df4Rate['cl3d_isobdt_score'] > ISObdtWP05
            df4Rate['cl3d_isobdt_passWP10_PUWP{0}'.format(PUWP)] = df4Rate['cl3d_isobdt_score'] > ISObdtWP10
            df4Rate['cl3d_isobdt_passWP15_PUWP{0}'.format(PUWP)] = df4Rate['cl3d_isobdt_score'] > ISObdtWP15
            del full, model_iso, ISObdtWP01, ISObdtWP05, ISObdtWP10

        del features_isoQCDrej, model_isoQCDrej, WP01_isoQCDrej, WP05_isoQCDrej, WP10_isoQCDrej # free memory

        #######################################################################################################################################
        # DM SORTING
        features_DMsort = ['cl3d_pt_c1', 'cl3d_pt_c2', 'cl3d_pt_c3', 'cl3d_abseta', 'cl3d_showerlength', 'cl3d_coreshowerlength', 'cl3d_firstlayer', 'cl3d_seetot', 'cl3d_seemax', 'cl3d_spptot', 'cl3d_sppmax', 'cl3d_szz', 'cl3d_srrtot', 'cl3d_srrmax', 'cl3d_srrmean', 'cl3d_hoe', 'cl3d_meanz']
    ...
    if args.doOnlyRate:
        indir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/hdf5dataframes/rateEvluation'
        inFileMinbias = {
            'threshold'    : indir+'/Minbias_PU200_th_rateEvaluated.hdf5',
            'mixed'        : indir+'/'
        }
    ...
    mapping_indir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/pklModels/mapping_C1fullC2C3_fullPUnoPt_fullISO'
    plotdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT_redEta/plots/rateEvaluation_C1fullC2C3_fullPUnoPt_fullISO'
    os.system('mkdir -p '+plotdir)

    mappings_dict = {
        'threshold' : {
            'PUWP99' : {
                'ISOWP01' : mapping_indir+'/Tau_PU200_th_PUWP99_ISOWP01_mapping.pkl',
                'ISOWP05' : mapping_indir+'/Tau_PU200_th_PUWP99_ISOWP05_mapping.pkl',
            ...
            'PUWP95' : {
                'ISOWP01' : mapping_indir+'/Tau_PU200_th_PUWP95_ISOWP01_mapping.pkl',
                'ISOWP05' : mapping_indir+'/Tau_PU200_th_PUWP95_ISOWP05_mapping.pkl',
            ...
            'PUWP90' : {
                'ISOWP01' : mapping_indir+'/Tau_PU200_th_PUWP90_ISOWP01_mapping.pkl',
                'ISOWP05' : mapping_indir+'/Tau_PU200_th_PUWP90_ISOWP05_mapping.pkl',
        ...
        'mixed' : {
            'PUWP99' : {
                'ISOWP01' : mapping_indir+'',
                'ISOWP05' : mapping_indir+'',
    ...
    df4Rate.reset_index(inplace=True)
    events_total = np.unique(df4Rate['event']).shape[0]
    ...
    for PUWP in ['99', '95', '90']:
        ...
        for ISOWP in ['01', '05', '10', '15']:
            ...
            rate = []
            for thr in tmp['cl3d_pt_c3']:
                rate.append( float( len(tmp.query('cl3d_pt_c3>{0}'.format(float(thr)))) ) )

            rates_online['singleTau']['PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)] = ( np.sort(tmp['cl3d_pt_c3']), np.array(rate)/events_total*events_frequency )
            rates_offline['singleTau']['pt95_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)] = ( np.sort(tmp['cl3d_pt95_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)]), np.array(rate)/events_total*events_frequency )
            rates_offline['singleTau']['pt90_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)] = ( np.sort(tmp['cl3d_pt90_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)]), np.array(rate)/events_total*events_frequency )
            rates_offline['singleTau']['pt50_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)] = ( np.sort(tmp['cl3d_pt50_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)]), np.array(rate)/events_total*events_frequency )
            ...
            rates_online['doubleTau']['PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)] = ( np.sort(tmp['cl3d_pt_c3']), np.array(rate)/events_total*events_frequency )
            rates_offline['doubleTau']['pt95_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)] = ( np.sort(tmp['cl3d_pt95_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)]), np.array(rate)/events_total*events_frequency )
            rates_offline['doubleTau']['pt90_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)] = ( np.sort(tmp['cl3d_pt90_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)]), np.array(rate)/events_total*events_frequency )
            rates_offline['doubleTau']['pt50_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)] = ( np.sort(tmp['cl3d_pt50_PUWP{0}_ISOWP{1}'.format(PUWP,ISOWP)]), np.array(rate)/events_total*events_frequency )
