...
if __name__ == "__main__" :
    ...
    outdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/hdf5dataframes/PUrejected_noCal_fullPUnoPt'
    plotdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/plots/PUrejection_noCal_fullPUnoPt'
    model_outdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/pklModels/PUrejection_noCal_fullPUnoPt'
    os.system('mkdir -p '+indir+'; mkdir -p '+outdir+'; mkdir -p '+plotdir+'; mkdir -p '+model_outdir)

    # set output to go both to terminal and to file
    sys.stdout = Logger("/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/pklModels/PUrejection_noCal_fullPUnoPt/performance.log")
    ...
    for name in feNames_dict:
        ...
        for threshold in [0,30]: # consider the no threshold and the 30GeV offline threshold cases
            ...
            for PUWP in [99,95,90]:
                ...
                eff_Tau = effVSpt_Tau_dict[name]['efficiency_PUWP{0}_at{1}GeV'.format(PUWP,threshold)]
                eff_err_low_Tau = effVSpt_Tau_dict[name]['efficiency_PUWP{0}_err_low_at{1}GeV'.format(PUWP,threshold)]
                eff_err_up_Tau = effVSpt_Tau_dict[name]['efficiency_PUWP{0}_err_up_at{1}GeV'.format(PUWP,threshold)]

                plt.errorbar(x_Tau,eff_Tau,xerr=1,yerr=[eff_err_low_Tau,eff_err_up_Tau],ls='None',label=r'PUWP = {0}'.format(PUWP),color=col[PUWP],lw=2,marker='o',mec=col[PUWP], alpha=0.5)
        ...
        for threshold in [0,30]: # consider the no threshold and the 30GeV offline threshold cases
            ...
            for PUWP in [99,95,90]:
                # all values for turnON curves
                eff_Tau = effVSeta_Tau_dict[name]['efficiency_PUWP{0}_at{1}GeV'.format(PUWP,threshold)]
                eff_err_low_Tau = effVSeta_Tau_dict[name]['efficiency_PUWP{0}_err_low_at{1}GeV'.format(PUWP,threshold)]
                eff_err_up_Tau = effVSeta_Tau_dict[name]['efficiency_PUWP{0}_err_up_at{1}GeV'.format(PUWP,threshold)]

                plt.errorbar(x_Tau,eff_Tau,xerr=0.05,yerr=[eff_err_low_Tau,eff_err_up_Tau],ls='None',label=r'PUWP = {0}'.format(PUWP),color=col[PUWP],lw=2,marker='o',mec=col[PUWP], alpha=0.5)
