...
if __name__ == "__main__" :
    ...
    outdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/hdf5dataframes/visualization'
    ...
    if args.doHH:
        ...
        outFileHH_dict = {
            'threshold'    : outdir+'/GluGluHHTo2b2Tau_PU200_th_visualization.hdf5',
            'supertrigger' : outdir+'/',
            'bestchoice'   : outdir+'/',
            'bestcoarse'   : outdir+'/',
            'mixed'        : outdir+'/'
    ...
    if args.doTau:
        ...
        outFileTau_dict = {
            'threshold'    : outdir+'/RelValTenTau_PU200_th_visualization.hdf5',
            'supertrigger' : outdir+'/',
            'bestchoice'   : outdir+'/',
            'bestcoarse'   : outdir+'/',
            'mixed'        : outdir+'/'
    ...
    if args.doNu:
        ...
        outFileNu_dict = {
            'threshold'    : outdir+'/RelValNu_PU200_th_visualization.hdf5',
            'supertrigger' : outdir+'/',
            'bestchoice'   : outdir+'/',
            'bestcoarse'   : outdir+'/',
            'mixed'        : outdir+'/'
    ...
    if args.doQCD:
        ...
        outFileQCD_dict = {
            'threshold'    : outdir+'/QCD_PU200_th_visualization.hdf5',
            'supertrigger' : outdir+'/',
            'bestchoice'   : outdir+'/',
            'bestcoarse'   : outdir+'/',
            'mixed'        : outdir+'/'
    ...
    # TBranches to be stored containing the TCs' info
    branches_tc = ['event', 'tc_layer', 'tc_energy', 'tc_x', 'tc_y', 'tc_z']
    ...
    for name in feNames_dict:
        ...
        if args.doHH:
            ...
            print('** INFO: saving TCs for visualization for ' + inFileHH_dict[name])
            dfHH_visualization = root_pandas.read_root(inFileHH_dict[name], key=treename, columns=branches_tc, flatten=False)

            print(dfHH)

            # events = dfHH.groupby('eventID')['eventID'].values
            # print(events)

            # print('** INFO: saving file ' + outFileHH_dict[name])
            # store_hh = pd.HDFStore(outFileHH_dict[name], mode='w')
            # store_hh[name] = dfHH_visualization
            # store_hh.close()
