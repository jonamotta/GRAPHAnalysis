...
if __name__ == "__main__" :
    ...
    outdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/hdf5dataframes/DMsorted_skimPUnoPt{0}_skimISO{0}{1}_test'.format(tag1, tag2)
    plotdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/plots/DMsorting_skimPUnoPt{0}_PUWP{2}_skimISO{0}{1}_ISOWP{3}_test'.format(tag1, tag2, args.PUWP, args.ISOWP)
    model_dict_outdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/pklModels/DMsorting_skimPUnoPt{0}_skimISO{0}{1}_test'.format(tag1, tag2)
    os.system('mkdir -p '+indir+'; mkdir -p '+outdir+'; mkdir -p '+plotdir+'; mkdir -p '+model_dict_outdir)

    # set output to go both to terminal and to file
    sys.stdout = Logger("/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/pklModels/DMsorting_skimPUnoPt{0}_skimISO{0}{1}_test/performance_PUWP{2}_ISOWP{3}.log".format(tag1, tag2, args.PUWP,args.ISOWP))
    ...
    # BDT hyperparameters
    params_dict = {}
    params_dict['eval_metric']        = 'mlogloss'
    ...
    for name in feNames_dict:
        ...
        print('\n** INFO: training BDT')
        X_train, X_test, y_train, y_test = train_test_split(dfTr[features], dfTr['gentau_decayMode'], test_size=0.3)
        classifier = xgb.XGBClassifier(params_dict, n_estimators=num_trees, objective='multi:softprob', use_label_encoder=False)
        model_dict[name] = classifier.fit(X_train, y_train)
        ...
        dfVal['cl3d_predDM'] = model_dict[name].predict(dfVal[features])

        print('\n** INFO: doing prediction')
