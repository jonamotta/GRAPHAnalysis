# THIS DM SORTING HAS 4 TAU CATEGORIES AND 1 QCD CATEGORY
#       - 1prong
#       - 1prong+pi0
#       - 3prong
#       - 3prong+pi0
...
if __name__ == "__main__" :
    ...
    plotdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/plots/DMsorting_{0}'.format(PUbdtWP)
    ...
    outFileTraining_dict = {
        'threshold'    : outdir+'/Training_PU200_th_PUWP{0}_DMsorted.hdf5'.format(args.WP),
        'supertrigger' : outdir+'/',
        'bestchoice'   : outdir+'/',
        'bestcoarse'   : outdir+'/',
        'mixed'        : outdir+'/'
    ...
    outFileValidation_dict = {
        'threshold'    : outdir+'/Validation_PU200_th_PUWP{0}_DMsorted.hdf5'.format(args.WP),
        'supertrigger' : outdir+'/',
        'bestchoice'   : outdir+'/',
        'bestcoarse'   : outdir+'/',
        'mixed'        : outdir+'/'
    ...
    for name in feNames_dict:
        ...
        dfTraining_dict[name] = dfTraining_dict[name].query('(gentau_decayMode==0 or gentau_decayMode==1 or gentau_decayMode==10 or gentau_decayMode==11 or gentau_decayMode==-2) and cl3d_pubdt_pass{0}==True'.format(PUbdtWP)) # drop DM 5-6

        dfValidation_dict[name] = dfValidation_dict[name].query('(gentau_decayMode==0 or gentau_decayMode==1 or gentau_decayMode==10 or gentau_decayMode==11 or gentau_decayMode==-2) and cl3d_pubdt_pass{0}==True'.format(PUbdtWP)) # drop DM 5-6
        ...
        dfTraining_dict[name]['gentau_decayMode'] = dfTraining_dict[name]['gentau_decayMode'].replace([10], 2) # tag 3-prong
        dfTraining_dict[name]['gentau_decayMode'] = dfTraining_dict[name]['gentau_decayMode'].replace([11], 3) # tag 3-prong+pi
        dfTraining_dict[name]['gentau_decayMode'] = dfTraining_dict[name]['gentau_decayMode'].replace([-2], 4) # tag qcd

        dfValidation_dict[name]['gentau_decayMode'] = dfValidation_dict[name]['gentau_decayMode'].replace([10], 2) # tag 3-prong(+pi)  
        dfValidation_dict[name]['gentau_decayMode'] = dfValidation_dict[name]['gentau_decayMode'].replace([11], 3) # tag 3-prong(+pi)  
        dfValidation_dict[name]['gentau_decayMode'] = dfValidation_dict[name]['gentau_decayMode'].replace([-2], 4) # tag qcd

        #dfNuTraining_dict[name]['gentau_decayMode'] = dfNuTraining_dict[name]['gentau_decayMode'].replace([-1], 4) # tag residual pileup as QCD
        #dfNuValidation_dict[name]['gentau_decayMode'] = dfNuValidation_dict[name]['gentau_decayMode'].replace([-1], 4) # tag residual pileup as QCD
        ...
        dfTraining_dict[name]['cl3d_probDM4'] = probas_train[:,4]
        ...
        dfValidation_dict[name]['cl3d_probDM4'] = probas_valid[:,4]
        ...
        #dfNuTraining_dict[name]['cl3d_probDM4'] = probasNu[:,4]
        #del probasNu
        ...
        #dfNuValidation_dict[name]['cl3d_probDM4'] = probasNu[:,4]
        ...
        if args.doPlots:
            for name in feNames_dict:
                ...
                for var in features_dict:
                    ...
                    plt.hist(dfTrainingDM0_dict[name][var], density=True, bins=np.arange(features_dict[var][1][0],features_dict[var][1][1],(features_dict[var][1][1]-features_dict[var][1][0])/features_dict[var][1][2]),  label=r'1-prong', color='limegreen',  histtype='step', lw=2, alpha=0.8)
                    ...
                    plt.hist(dfTrainingDM10_dict[name][var], density=True, bins=np.arange(features_dict[var][1][0],features_dict[var][1][1],(features_dict[var][1][1]-features_dict[var][1][0])/features_dict[var][1][2]),  label=r'3-prong', color='red',  histtype='step', lw=2, alpha=0.8)
                    ...
                    plt.hist(dfTrainingQCD_dict[name][var], density=True, bins=np.arange(features_dict[var][1][0],features_dict[var][1][1],(features_dict[var][1][1]-features_dict[var][1][0])/features_dict[var][1][2]), label=r'QCD background', color='cyan',  histtype='step', lw=2, alpha=0.8)
                ...
                plt.xticks([0,1,2,3,4], ('1-prong', r'1-prong$+\pi_0$', r'3-prong', r'3-prong$+\pi_0$', 'QCD'))
                plt.yticks([0,1,2,3,4], ('1-prong', r'1-prong$+\pi_0$', r'3-prong', r'3-prong$+\pi_0$', 'QCD'))
                plt.xlim(-0.5, 4.5)
                plt.ylim(-0.5, 4.5),
                ...
                plt.figure(figsize=(15,10))
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==0].cl3d_probDM0, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'1-prong', density=True)
                ...
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==2].cl3d_probDM0, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'3-prongs', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==3].cl3d_probDM0, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'3-prongs$+\pi_0$', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==4].cl3d_probDM0, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'QCD', density=True)
                ...
                plt.figure(figsize=(15,10))
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==0].cl3d_probDM1, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'1-prong', density=True)
                ...
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==2].cl3d_probDM1, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'3-prongs', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==3].cl3d_probDM1, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'3-prongs$+\pi_0$', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==4].cl3d_probDM1, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'QCD', density=True)
                ...
                plt.figure(figsize=(15,10))
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==0].cl3d_probDM2, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'1-prong', density=True)
                ...
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==2].cl3d_probDM2, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'3-prongs', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==3].cl3d_probDM2, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'3-prongs$+\pi_0$', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==4].cl3d_probDM2, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'QCD', density=True)
                ...
                plt.legend(fontsize=22)
                plt.title(r'Probabilities 3-prong category')
                plt.ylabel(r'Entries')
                plt.savefig(plotdir+'/DM2_prob_'+PUbdtWP+'_'+name+'.pdf')
                plt.close()

                plt.figure(figsize=(15,10))
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==0].cl3d_probDM3, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'1-prong', density=True)
                ...
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==2].cl3d_probDM3, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'3-prongs', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==3].cl3d_probDM3, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'3-prongs$+\pi_0$', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==4].cl3d_probDM3, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'QCD', density=True)
                #plt.hist(dfNuValidation_dict[name].cl3d_probDM2, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'Residual PU', density=True)                
                plt.legend(fontsize=22)
                ...
                plt.figure(figsize=(15,10))
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==0].cl3d_probDM4, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label='1-prong', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==1].cl3d_probDM4, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'1-prong$+\pi_0$', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==2].cl3d_probDM4, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'3-prongs', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==3].cl3d_probDM4, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'3-prongs$+\pi_0$', density=True)
                plt.hist(dfValidation_dict[name][dfValidation_dict[name].gentau_decayMode==4].cl3d_probDM4, bins=np.arange(0., 1., 0.05), histtype='step', lw=2, label=r'QCD', density=True)
                ...
                plt.ylabel(r'Entries')
                plt.savefig(plotdir+'/DM4_prob_'+PUbdtWP+'_'+name+'.pdf')
                plt.close()
