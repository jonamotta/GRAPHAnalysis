...
def train_xgb(dfTr, features, output, hyperparams, num_trees):
    ...
    # booster = xgb.XGBClassifier(hyperparams, use_label_encoder=False)
    booster = xgb.XGBClassifier(use_label_encoder=False, objective='binary:logistic', booster='gbtree', 
                                eval_metric='logloss', reg_alpha=9, reg_lambda=5, max_depth=4, learning_rate=0.35, 
                                subsample=0.22, colsample_bytree=0.7, n_estimators=num_trees)
    booster.fit(X_train, y_train)
    X_train['bdt_output'] = booster.predict_proba(X_train)[:,1]
    fpr_train, tpr_train, threshold_train = metrics.roc_curve(y_train, X_train['bdt_output'])
    X_test['bdt_output'] = booster.predict_proba(X_test)[:,1]
...
if __name__ == "__main__" :
    ...
    outdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/hdf5dataframes/PUrejected_skimPUnoPt{0}_class'.format(tag)
    plotdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/plots/PUrejection_skimPUnoPt{0}_class'.format(tag)
    model_outdir = '/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/pklModels/PUrejection_skimPUnoPt{0}_class'.format(tag)
    os.system('mkdir -p '+indir+'; mkdir -p '+outdir+'; mkdir -p '+plotdir+'; mkdir -p '+model_outdir)

    # set output to go both to terminal and to file
    sys.stdout = Logger("/home/llr/cms/motta/HGCAL/CMSSW_11_1_0/src/GRAPHAnalysis/L1BDT/pklModels/PUrejection_skimPUnoPt{0}_class/performance.log".format(tag))
