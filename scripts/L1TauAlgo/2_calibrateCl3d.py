...
if __name__ == "__main__" :
    ...
    for name in feNames_dict:
        ...
        dfQCDValidation_dict[name] = dfValidation_dict[name].query('gentau_decayMode==-2 and cl3d_pt>{0} and((genjet_eta>1.6 and genjet_eta<2.9) or (genjet_eta<-1.6 and genjet_eta>-2.9) and cl3d_isbestmatch==True)'.format(args.ptcut))
        ...
        if args.doPlots:    
            ...
            plt.hist(dfValidationDM10_dict[name]['cl3d_response_c2'], bins=np.arange(0., 2., 0.03),  label=r'3-prong, mean: {0}, RMS: {1}'.format(round(dfValidationDM10_dict[name]['cl3d_response_c2'].mean(),3), round(dfValidationDM10_dict[name]['cl3d_response_c2'].std(),3)), color='red',  histtype='step', lw=2)
