...
if __name__ == "__main__" :
    ...
    for name in feNames_dict:
        ...
        dfMergedTraining_dict[name]['gentau_decayMode'].replace([1,6,11], 1, inplace=True) # prong+pi decays
        dfMergedTraining_dict[name]['gentau_decayMode'].replace([4], 2, inplace=True) # qcd

        dfMergedValidation_dict[name]['gentau_decayMode'].replace([0,5,10], 0, inplace=True) # prong decays
        dfMergedValidation_dict[name]['gentau_decayMode'].replace([1,6,11], 1, inplace=True) # prong+pi decays
        dfMergedValidation_dict[name]['gentau_decayMode'].replace([4], 2, inplace=True) # qcd
        ...
        dfTau_dict[name]['gentau_decayMode'].replace([1,6,11], 1, inplace=True)
        dfHH_dict[name]['gentau_decayMode'].replace([1,6,11], 1, inplace=True)
        dfQCD_dict[name]['gentau_decayMode'].replace([1,6,11], 1, inplace=True)
        dfTau_dict[name]['gentau_decayMode'].replace([4], 2, inplace=True)
        dfHH_dict[name]['gentau_decayMode'].replace([4], 2, inplace=True)
        dfQCD_dict[name]['gentau_decayMode'].replace([4], 2, inplace=True)
        ...
        if args.doPlots:
            for name in feNames_dict:
                ...
                plt.xticks([0,1,2], ('prong', r'prong$+\pi_0$', 'QCD'))
                plt.yticks([0,1,2], ('prong', r'prong$+\pi_0$', 'QCD'))
                plt.xlim(-0.5, 2.5)
                plt.ylim(-0.5, 2.5),
